...
CVscore.RDA <- function(param, profiles, ptype, mu.K, Sig.K, prob.cl)  {
  a <- param[1]
  b <- param[2]
  ...
  for (i in 1:n)  {
    ...
    for (k in 1:K)  {
      ...
      likelihood[k,] <- dmvnorm(profiles[i,,], mu.K[,k],Sig.K[,,k])
#       if (ptype[i]==k)  {
...
#prior.probs=array(0,dim=c(length(unique(months)),length(stations),4))
prob.cl <- array(dim=c(9,length(stations),K))
prob.fcst.climo <- matrix(NA,length(station.ind),4)

for(st in 1:length(stations)){
  
  if(st%%100==0){print(st)}	
  
  for(mm in 1:length(unique(months))){
    mon=sort(unique(months))[mm]
    #Finding the right stations	
    station.i=which(station.ind==st)
    ...
    rain.nn=length(which(ptype[rows.needed]=="2"))
    snow.nn=length(which(ptype[rows.needed]=="1"))
    pellet.nn=length(which(ptype[rows.needed]=="3"))
    ice.nn=length(which(ptype[rows.needed]=="4"))
    
    prob.cl[mm,st,1:4]=c(rain.nn,snow.nn,pellet.nn,ice.nn)/length(rows.needed)		
    prob.fcst.climo[rows.needed,] <- matrix(prob.cl[mm,st,],length(rows.needed),4,byrow=TRUE)
  }
}
...
for (iyear in 1:12)  {   # loop over the 12 cool seasons 2001-2012, use the previous 5 cool seasons for training
  ...
  Twb.prof.test <- Twb.prof[verif.ind,1:d]
...
#   }
  ...
  # Calculate classification probabilities for verification cases using the fitted regularization parameters
   for (i in 1:length(verif.ind))  {
     X <- t(Twb.prof.test[i,])
     #print(dim(X))
    #X <- matrix(Twb.prof[i,1:d],d,1)
    for (k in 1:K)  {
      
      likelihood[k,] <- dmvnorm(X,mu.K[,k],Sigma.K[,,k])
#       proj.R <- crossprod(v.K[,,k],sweep(X,1,mu.K[,k],"-"))
