...
CVscore.RDA <- function(param, profiles, ptype, mu.K, e.K, v.K, prob.cl)  {
  ...
  BS.train[iyear]=sum(BS.rda)
  BS.clim.train[iyear]=sum(BS.climo)
  BSS.train[iyear]= sum(1-BS.rda/BS.climo)
...
write.table(cbind(BS.train,BS.clim.train,BSS.train),"Results/Brier_Scores_train.txt")
...
prob.forecast <- read.table(file="michael.txt", header=T)
prob.hats = read.table(file="baseline_classification.txt", header=T)
prob.forecast[,5]<-prob.hats[,5]
...
png("~/Desktop/reliability.png", width=2100, height=2100, res=300)
par(new=F)
 #reliability.plot(n[1,],x[1,]/n[1,],y[1,]/n[1,])
reliability(prob.fcst.rda)
dev.off()
par(mfrow=c(1,1))
